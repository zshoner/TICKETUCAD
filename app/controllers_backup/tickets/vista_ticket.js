...
function apiFetch(url, options = {}) {
    ...
    opts.headers = Object.assign({}, opts.headers || {}, {
        ...
        'Accept': opts.headers && opts.headers.Accept ? opts.headers.Accept : 'application/json'
    });
...
async function cargarDetalleTicket(ticketId) {
    ...
    try {
        const [resTicket, resEst, resUsu, resCat] = await Promise.all([
            apiFetch(`/TICKETUCAD/app/models/php/obtener_ticket.php?id=${encodeURIComponent(ticketId)}`),
            apiFetch('/TICKETUCAD/app/models/php/obtener_estados.php'),
            apiFetch('/TICKETUCAD/app/models/php/obtener_usuarios.php'),
            apiFetch('/TICKETUCAD/app/models/php/obtener_categorias.php'),
        ]);
...
function enlazarControles(ticketId) {
    ...
    e.addEventListener('change', function () {
        ...
        msg.className = '';
        apiFetch('/TICKETUCAD/app/models/php/actualizar_estado.php', { method: 'POST', body: fd })
            .then(() => {
    ...
    c.addEventListener('change', function () {
        ...
        msg.className = '';
        apiFetch('/TICKETUCAD/app/models/php/actualizar_categoria.php', { method: 'POST', body: fd })
            .then(r => r.json())
            .then(data => {
                if (data.success) {
    ...
    a.addEventListener('change', function () {
        ...
        msg.className = '';
        apiFetch('/TICKETUCAD/app/models/php/actualizar_asignacion.php', { method: 'POST', body: fd })
            .then(() => {
...
async function cargarMensajes(ticketId) {
    ...
    try {
        const res = await apiFetch(`/TICKETUCAD/app/models/php/obtener_respuestas_ticket.php?id=${encodeURIComponent(ticketId)}`);
...
async function enviarRespuesta(ticketId) {
    ...
    try {
        ...
        const res = await apiFetch('/TICKETUCAD/app/models/php/guardar_respuesta_ticket.php', {
            method: 'POST',
            body: fd
        });
        const data = await res.json();
...
async function cerrarTicket(ticketId) {
    ...
    try {
        const fd = new FormData();
        fd.append('ticket_id', ticketId);
        const res = await apiFetch('/TICKETUCAD/app/models/php/cerrar_ticket.php', { method: 'POST', body: fd });
        const data = await res.json();
        if (!data.success) {
